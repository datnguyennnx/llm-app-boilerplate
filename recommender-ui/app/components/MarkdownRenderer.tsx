...
export const MarkdownRenderer = ({
    ...
    const components: Components = useMemo(
        () => ({
            h1: ({ ...props }) => (
                <h1
                    className="text-3xl font-bold my-0 text-indigo-800"
                    {...props}
                />
            ),
            h2: ({ ...props }) => (
            ...
            h3: ({ ...props }) => (
                <h3
                    className="text-xl font-medium my-0 text-indigo-600"
                    {...props}
                />
            ),
            h4: ({ ...props }) => (
                <h4
                    className="text-lg font-medium my-0 text-indigo-500"
                    {...props}
                />
            ),
            p: ({ ...props }) => (
            ...
            pre({ children }: any) {
                ...
                const handleCopy = async () => {
                    if (codeRef.current) {
                        const codeElement =
                            codeRef.current.querySelector('code')
                        if (codeElement) {
